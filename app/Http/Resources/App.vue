...
| Products
|--------------------------------------------------------------------------
| محصولات از API عمومی فروشگاه دریافت می‌شوند.
*/
...
if (state.subcategories.length) {
 ...
 if (state.color) {
    ...
    if (state.frameMaterial) {
        result = result.filter(
            (product) =>
                product.attributes?.['frame-material']?.some(
...
if (
    ...
    if (
        state.frameMaterial &&
        !availableFrameMaterials.value.some(
            (item) =>
                item.value === state.frameMaterial
        )
    ) {
        state.frameMaterial = null;
    }
...
async function loadProducts() {
    try {
        const firstResponse = await axios.get('/api/products', {
            params: { page: 1 },
        });

        const firstPageProducts = firstResponse.data?.data || [];
        const lastPage = Number(
            firstResponse.data?.meta?.last_page || 1
        );

        if (lastPage <= 1) {
            products.value = firstPageProducts;
            return;
        }
...
async function loadCategoryFilters(categorySlug) {
    try {
        const response = await axios.get(
            `/api/categories/${categorySlug}/filters`
        );
        ...
        >
            {{ material.label }}
                ...
                <div
                    ...
                    class="border-b border-black/10 py-6 dark:border-white/10"
                >
                    <h3 class="mb-4 text-xs font-semibold">
                        جنس فریم
                    </h3>
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button
                            v-for="frameMaterial in availableFrameMaterials"
                            :key="`mobile-frame-material-${frameMaterial.id}`"
                            type="button"
                            class="filter-option rounded-full border border-black/10 px-3 py-2 text-[10px] dark:border-white/10"
                            :class="{
                                active:
